<script lang="ts">
  ...
  const checkKey = (key: string) => {
    ...
    if (Object.keys(settings.minecraft.jvm_settings.env_vars).includes(key)) {
    ...
    {#if settings}
      <div class="flex items-center gap-2">
        ...
        <Input
          ...
          value={settings.minecraft.jvm_settings.mem_max ?? 1024}
          min={512}
          max={settings.system_max_mem}
          step={64}
          onfocusout={(e) => {
            const value = parseInt((e.target as HTMLInputElement).value, 10);
            if (value !== settings.minecraft.jvm_settings.mem_max) {
        ...
        <Slider
          ...
          max={settings.system_max_mem}
          onValueChange={(value) => {
            if (value === settings.minecraft.jvm_settings.mem_max) return;
            saveJavaSettings({ mem_max: value });
          }}
        />
      </div>
      ...
      <Input
        ...
        value={settings.minecraft.jvm_settings.args.join(' ') ?? ''}
        oninput={(e) => {
          const value = (e.target as HTMLInputElement).value
            .trim()
            .split(/\s+/)
            .filter(Boolean);
          if (value !== settings.minecraft.jvm_settings.args) {
            saveJavaSettings({ args: value });
          }
        }}
      />
      <Label for="jvm-env" class="whitespace-nowrap"
        >Environment Variables</Label
      >
      {#if Object.keys(settings.minecraft.jvm_settings.env_vars).length === 0}
      ...
      {:else}
        <ScrollArea.ScrollArea class="min-h-0">
          <div class="flex flex-col gap-2">
            {#each Object.entries(settings.minecraft.jvm_settings.env_vars).sort( ([keyA], [keyB]) => keyA.localeCompare(keyB) ) as [key, value]}
              <div class="flex items-center gap-2">
                <Input
                  type="text"
                  ...
                  onfocusout={(e) => {
                    ...
                    if (newKey !== key) {
                      const newEnv = {
                        ...settings.minecraft.jvm_settings.env_vars
                      };
                      delete newEnv[key];
                      newEnv[newKey] = value;
      ...
      <div class="flex items-center gap-2">
        ...
        <Button
          ...
          onclick={() => {
            ...
            if (!checkKey(newKey)) return;
            const newEnv = { ...settings.minecraft.jvm_settings.env_vars };
